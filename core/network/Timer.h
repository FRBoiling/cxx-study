...
namespace boil {
    ...
    template<typename ValueType>
    inline
    Timer<ValueType>::Timer(EventLoop *loop, uint64_t timeout, uint64_t repeat, TimerCallback callback, ValueType value)
            :started_(false),
    ...
    template<typename ValueType>
    inline void Timer<ValueType>::closeComplete() {
        if (closeComplete_)
            closeComplete_(this);
    }
